...
void Supply::supplyProcess() {
    while (true) {
            ...
            water = true;
            wait(17, SC_NS);  // Assume water is available for 10 seconds
            waterAvailable.write(false);
            SC_REPORT_INFO("Supply", "Out of water");
            water = false;
            wait(2.5, SC_NS);  // Assume out of water for the next 20 seconds
            SC_REPORT_INFO("Supply", "Water is available");
            water = true;
            wait(10, SC_NS);  // Assume water is available for 10 seconds
            waterAvailable.write(true);
    }
}
